...
@implementation SamiTrialsDeviceTypeArray
        ...
        if([deviceTypes_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)deviceTypes_dict count] > 0) {
                for (NSDictionary* dict in (NSArray*)deviceTypes_dict) {
                    SamiTrialsDeviceType* d = [[SamiTrialsDeviceType alloc] initWithValues:dict];
                    [objs addObject:d];
                }
...
-(NSDictionary*) asDictionary {
    ...
    if(_deviceTypes != nil){
        if([_deviceTypes isKindOfClass:[NSArray class]]){
            ...
            for( SamiTrialsDeviceType *deviceTypes in (NSArray*)_deviceTypes) {
